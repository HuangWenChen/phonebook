...
int main(void)
{
    ...
    double hash2_opt_sum_a = 0.0, hash2_opt_sum_f = 0.0, hash2_opt_a, hash2_opt_f;
    for (i = 0; i < 100; i++) {
        if (feof(fp)) {
            printf("ERROR: You need 100 datum instead of %d\n", i);
            printf("run 'make run' longer to get enough information\n\n");
        ...
        fscanf(fp, "%s %s %lf %lf\n", append, find,&hash2_opt_a, &hash2_opt_f);
        hash2_opt_sum_a += hash2_opt_a;
        hash2_opt_sum_f += hash2_opt_f;
    }

    fprintf(output, "append() %lf %lf %lf %lf\n",orig_sum_a / 100.0, opt_sum_a / 100.0, hash_opt_sum_a / 100.0, hash2_opt_sum_a / 100.0);
    fprintf(output, "findName() %lf %lf %lf %lf", orig_sum_f / 100.0, opt_sum_f / 100.0, hash_opt_sum_f / 100.0, hash2_opt_sum_f / 100.0);
    fclose(output);
    fclose(fp);
    return 0;
}
